...
private:
    class TxEngine {
        ...
        static constexpr u32 delay_to_carrier_cnt(u32 delay) {
    ...
    public:
        ...
        static void start(u32 raw_frame, u16 mark_us_, u16 space_us_) {
            start(raw_frame, mark_us_, space_us_, DATA_BIT_CNT);
        }

        static void start_repeat(u16 mark_us_, u16 space_us_) {
            start(0, mark_us_, space_us_, 0);
        }
        ...
        //----------------------------------------------------------------------
        static void start(u32 raw_frame, u16 mark_us_, u16 space_us_, u8 bit_cnt_) {
            tx_off();
            ...
            load_symbol(
                delay_to_carrier_cnt(microsecondsToClockCycles(mark_us_)),
                microsecondsToClockCycles(space_us_)
            );

            setTimer1Callback(on_timer1);
        }

        //----------------------------------------------------------------------
        static u32 IRAM_ATTR on_timer1() {
            const u32 now = now_cycles();
            if (!is_due(now)) {
                return timer_delay(next_edge_at - now);
            }

            if (carrier_cnt > 0) {
                return send_carrier(now);
            }
            ...
            switch (current_state) {
                case State::idle:
                    return idle();
                case State::symbol:
                    return finish_symbol(now);
                default:
                    return done();
            }
        }

        static bool IRAM_ATTR is_due(u32 now) {
            return next_edge_at == 0
                || static_cast<s32>(now - next_edge_at) >= 0;
        }

        static u32 IRAM_ATTR send_carrier(u32 now) {
            ...
            if (edge_is_on) {
                tx_on();
                return schedule(now, CARRIER_ON_DELAY);
            }
            else {
                tx_off();
                return schedule(now, CARRIER_OFF_DELAY);
            }
        }

        static u32 IRAM_ATTR finish_symbol(u32 now) {
            tx_off();
            ...
            return schedule(now, current_space_delay);
        }

        static void IRAM_ATTR load_next_symbol() {
            const bool is_one = (data & 1u) != 0;
            data >>= 1; // after data bits, zero shifts in for the stop bit
            --bit_cnt;

            load_symbol(
                delay_to_carrier_cnt(BIT_MARK_DELAY),
                is_one ? ONE_SPACE_DELAY : ZERO_SPACE_DELAY
            );
        }

        static void IRAM_ATTR load_symbol(u32 mark_carrier_cnt_, u32 space_delay_) {
            carrier_cnt = mark_carrier_cnt_;
            space_delay = space_delay_;
            state = State::symbol;
        }

        static u32 IRAM_ATTR idle() {
            tx_off();
            return wait_idle();
        }

        static u32 IRAM_ATTR done() {
            tx_off();
            state = State::done;
            return wait_idle();
        }

        static u32 IRAM_ATTR schedule(u32 now, u32 delay) {
            const u32 wait_delay = timer_delay(delay);
            next_edge_at = now + wait_delay;
            return wait_delay;
        }

        //----------------------------------------------------------------------
        static u32 IRAM_ATTR now_cycles() {
            u32 cycles;
        ...
        //----------------------------------------------------------------------
        static u32 IRAM_ATTR timer_delay(u32 delay) {
            return max(delay, DELAY_MIN);
        }

        static u32 IRAM_ATTR wait_idle() {
            next_edge_at = 0;
            return DELAY_MAX;
        }

        //----------------------------------------------------------------------
        static void IRAM_ATTR tx_on() {
